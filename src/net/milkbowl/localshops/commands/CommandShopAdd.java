...
public class CommandShopAdd extends Command {
    ...
    public boolean process() {
            ...
            if (matcher.find()) {
                ...
                if(LocalShops.getItemList().isDurable(itemStack)) {
                    ...
                    if (calcDurabilityPercentage(itemStack) > Config.getItemMaxDamage() && Config.getItemMaxDamage() != 0) {
                        sender.sendMessage(plugin.getResourceManager().getString(Messages.CMD_SHP_ADD_TOO_DAM, new String[] { "%ITEMNAME%" }, new String[] { item.name }));
                        sender.sendMessage(plugin.getResourceManager().getString(Messages.CMD_SHP_ADD_DMG_LESS_THAN, new String[] { "%DAMAGEVALUE%" }, new String[] { String.valueOf(Config.getItemMaxDamage()) }));
                        return true;
                    }
                } else {
        ...
        // Show add help
        sender.sendMessage(plugin.getResourceManager().getString(Messages.CMD_SHP_ADD_USAGE, new String[] { "%CMDLABEL%" }, new String[] { commandLabel }));
        return true;
    }
    ...
    private boolean shopAdd(Shop shop, ItemInfo item, int amount) {
        ...
        if (player != null) {
            ...
            if (playerItemCount < amount) {
                // Nag player
                log.info(plugin.getResourceManager().getString(Messages.CMD_SHP_ADD_INSUFFICIENT_INV, new String[] { "%ITEMCOUNT%" }, new Object[] { playerItemCount }));
                return false;
            }
        ...
        if (shop.containsItem(item)) {
            ...
            if (shop.isUnlimitedStock()) {
                // nicely message user
                sender.sendMessage(plugin.getResourceManager().getString(Messages.CMD_SHP_ADD_UNLIM_STOCK, new String[] { "%SHOPNAME%", "%ITEMNAME%" }, new Object[] { shop.getName(), item.name }));
                return true;
            }
            ...
            if (amount == 0) {
                // nicely message user
                sender.sendMessage(plugin.getResourceManager().getString(Messages.CMD_SHP_ADD_ALREADY_HAS, new String[] { "%SHOPNAME%", "%ITEMNAME%" }, new Object[] { shop.getName(), item.name }));
                return true;
            }
        }
        ...
        if (shop.isUnlimitedStock()) {
            sender.sendMessage(plugin.getResourceManager().getString(Messages.CMD_SHP_ADD_SUCCESS, new String[] { "%ITEMNAME%" }, new Object[] { item.name }));
        } else {
            shop.addStock(item.name, amount);
            shop.updateSigns(item.name);
            sender.sendMessage(plugin.getResourceManager().getString(Messages.CMD_SHP_ADD_STOCK_SUCCESS, new String[] { "%ITEMNAME%", "%STOCK%" }, new Object[] { item.name, shop.getItem(item.name).getStock() }));
        }
        
        if(amount == 0) {
            sender.sendMessage(plugin.getResourceManager().getString(Messages.CMD_SHP_ADD_READY0, new String[] { "%ITEMNAME%" }, new Object[] { item.name }));
            sender.sendMessage(plugin.getResourceManager().getString(Messages.CMD_SHP_ADD_READY1, new String[] { "%ITEMNAME%" }, new Object[] { item.name }));
            sender.sendMessage(plugin.getResourceManager().getString(Messages.CMD_SHP_ADD_READY2, new String[] { "%ITEMNAME%" }, new Object[] { item.name }));
        }
